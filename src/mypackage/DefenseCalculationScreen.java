...
public class DefenseCalculationScreen extends MainScreen implements FieldChangeListener {
	...
	public DefenseCalculationScreen() {
        ...
        rtfHeading = new RichTextField("Defense Calculation", RichTextField.TEXT_ALIGN_HCENTER){
		...
		gamesTiedEdit = new EditField("","",10, TextField.NO_LEARNING | TextField.NO_NEWLINE | TextField.NON_SPELLCHECKABLE ){
            ...
            gamesTiedEdit.setMargin(10,10,20,10);
	    gamesTiedLabel = new LabelField( "Games Tied in Goal" ){
            public void paint(Graphics g ){
                g.setColor(Color.WHITE);
                super.paint(g);
              }
		...
		if(field == calculateButton)
		{
			if(calculateButton.getLabel().equals("Calculate"))
			{
				...
				try {
					...
					else if(shotsOnGoal < 0 || goalsScored < 0 || gamesWon < 0 || gamesLost < 0 || gamesTied < 0)
					{
					...
					else
					{
						...
						stResult.append("Goalie Games Won Percentage : " + format.formatNumber(goalieWonPercentage,3) + "\n\n" +
								        "Goalie Saves : " + String.valueOf(goalieSaves) + "\n\n" +
								        "Saves Percentage : " + format.formatNumber(savesPercentage, 3) + "\n\n" +
